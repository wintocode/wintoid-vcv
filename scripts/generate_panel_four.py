#!/usr/bin/env python3
"""Generate Four panel SVG and C++ coordinate header.

Run from project root:
    python3 scripts/generate_panel.py

Outputs:
    res/Four.svg  — panel SVG background
...
def generate_coords_header():
    ...
    lines.append('')
    lines.append('namespace four_layout {')
    lines.append('')
...
if __name__ == '__main__':
    ...
    svg_path = os.path.join(project_dir, 'res', 'Four.svg')
    with open(svg_path, 'w') as f:
        f.write(generate_svg())
    print(f'Wrote {svg_path}')

    header_path = os.path.join(project_dir, 'src', 'Four', 'layout.h')
