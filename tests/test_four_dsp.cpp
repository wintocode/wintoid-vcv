...
{
    ASSERT_NEAR( four::coarse_ratio_from_index(0), 0.25f, 1e-6f );
}

TEST(coarse_ratio_index_1)
{
    ASSERT_NEAR( four::coarse_ratio_from_index(1), 0.5f, 1e-6f );
}

TEST(coarse_ratio_index_2)
{
    ASSERT_NEAR( four::coarse_ratio_from_index(2), 0.75f, 1e-6f );
}

TEST(coarse_ratio_index_3)
{
    // Index 3 = ratio 1.0
    ASSERT_NEAR( four::coarse_ratio_from_index(3), 1.0f, 1e-6f );
}

TEST(coarse_ratio_index_5)
{
    // Index 5 = ratio 2.0
    ASSERT_NEAR( four::coarse_ratio_from_index(5), 2.0f, 1e-6f );
}

TEST(coarse_ratio_index_64)
{
    // Index 64 = ratio 31.5 (formula: (idx-1)*0.5 for idx >= 3)
    ASSERT_NEAR( four::coarse_ratio_from_index(64), 31.5f, 1e-6f );
}

TEST(coarse_fixed_from_param)
{
    // Param 0.0 → 1 Hz, param 64.0 → 9999 Hz
    ASSERT_NEAR( four::coarse_fixed_from_param(0.0f), 1.0f, 0.1f );
    ASSERT_NEAR( four::coarse_fixed_from_param(64.0f), 9999.0f, 1.0f );
    // Monotonically increasing
    ASSERT( four::coarse_fixed_from_param(32.0f) > four::coarse_fixed_from_param(16.0f) );
}
